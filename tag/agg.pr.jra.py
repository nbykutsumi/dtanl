...
for ldist in lldist:
  ...
  for prtype in lprtype:
    for bstflag_tc in lbstflag_tc:
      ...
      tagdir_root = "/media/disk2/out/JRA25/sa.one.%s/6hr/tag/c%02dh.tc%02dh.%stc%04d.c%04d.%sf%04d"%(sresol, thdura_c, thdura_tc, bstflag_tc, dist_tc/1000.0, dist_c/1000.0, bstflag_f, dist_f/1000.0)
        ...
        if calcflag == True:
          ...
          for year in range(iyear, eyear+1):
            ...
            for mon in lmon:
              ...
              for day in range(iday, eday+1):
                ...
                for hour in lhour:
                  ...
                  for htag_inc in lhtag_inc:
                    ...
                    tagdir   = tagdir_root + "/%04d%02d"%(year_target, mon_target)
                    #tagname  = tagdir + "/tag.%stc%04d.c%04d.f%04d.%04d.%02d.%02d.%02d.sa.one"%(tctype, dist_tc, dist_c, dist_f, year_target,mon_target,day_target,hour_target)
                    tagname  = tagdir + "/tag.%04d.%02d.%02d.%02d.sa.one"%(year_target,mon_target,day_target,hour_target)
                  ...
                  # overlap: baroclinic
                  a2tag_te    = ma.masked_where(a2tagtmp_tc ==0.0, a2one)
                  a2tag_te    = ma.masked_where(a2tagtmp_c  ==0.0, a2tag_te).filled(0.0)

                  a2tag_tf    = ma.masked_where(a2tagtmp_tc ==0.0, a2one)
                  a2tag_tf    = ma.masked_where(a2tagtmp_fbc==0.0, a2tag_tf).filled(0.0)

                  a2tag_ef    = ma.masked_where(a2tagtmp_c  ==0.0, a2one)
                  a2tag_ef    = ma.masked_where(a2tagtmp_fbc==0.0, a2tag_ef).filled(0.0)

                  a2tag_tef   = ma.masked_where(a2tagtmp_tc ==0.0, a2one)
                  a2tag_tef   = ma.masked_where(a2tagtmp_c  ==0.0, a2tag_tef)
                  a2tag_tef   = ma.masked_where(a2tagtmp_fbc==0.0, a2tag_tef).filled(0.0)

                  #print year,mon,day,hour,a2tag_te.sum(), "AAAA",a2tag_tf.sum(), a2tag_ef.sum(), a2tag_tef.sum() , a2tagtmp_fbc.sum()
                  ##*****************************
                  ...
                  # overlap: baroclinic 
                  a2prtmp_te     = ma.masked_less(a2pr,0.0).filled(0.0) * a2tag_te
                  a2prtmp_tf     = ma.masked_less(a2pr,0.0).filled(0.0) * a2tag_tf
                  a2prtmp_ef     = ma.masked_less(a2pr,0.0).filled(0.0) * a2tag_ef
                  a2prtmp_tef    = ma.masked_less(a2pr,0.0).filled(0.0) * a2tag_tef
              ...
              # overlap:
              a2pr_te     = a2pr_te    *coef/ totaltimes 
              a2pr_tf     = a2pr_tf    *coef/ totaltimes 
              a2pr_ef     = a2pr_ef    *coef/ totaltimes 
              a2pr_tef    = a2pr_tef   *coef/ totaltimes 
              ...
              #-- save monthly data ---
              #-----------------------------
              sodir_root   = "/media/disk2/out/JRA25/sa.one.%s/6hr/tagpr/c%02dh.tc%02dh.%stc%04d.c%04d.%sf%04d"%(sresol, thdura_c, thdura_tc, bstflag_tc, dist_tc/1000.0, dist_c/1000.0, bstflag_f, dist_f/1000.0)
              sodirwn_root = "/media/disk2/out/JRA25/sa.one.%s/6hr/tagpr/wn.c%02dh.tc%02dh.%stc%04d.c%04d.%sf%04d"%(sresol, thdura_c, thdura_tc, bstflag_tc, dist_tc/1000.0, dist_c/1000.0, bstflag_f, dist_f/1000.0)
              ...
              sname_all  =  sodir  + "/pr.plain.%04d.%02d.%s.sa.one" %(year, mon, prtype)
              sname_tc   =  sodir  + "/pr.tc.%04d.%02d.%s.sa.one"    %(year, mon, prtype)
              sname_c    =  sodir  + "/pr.c.%04d.%02d.%s.sa.one"     %(year, mon, prtype)
              sname_fbc  =  sodir  + "/pr.fbc.%04d.%02d.%s.sa.one"   %(year, mon, prtype)
              sname_ot   =  sodir  + "/pr.ot.%04d.%02d.%s.sa.one"    %(year, mon, prtype)
              ...
              snamewn_all  =  sodirwn  + "/pr.wn.plain.%04d.%02d.%s.sa.one" %(year, mon, prtype)
              snamewn_tc   =  sodirwn  + "/pr.wn.tc.%04d.%02d.%s.sa.one"    %(year, mon, prtype)
              snamewn_c    =  sodirwn  + "/pr.wn.c.%04d.%02d.%s.sa.one"     %(year, mon, prtype)
              snamewn_fbc  =  sodirwn  + "/pr.wn.fbc.%04d.%02d.%s.sa.one"   %(year, mon, prtype)
              snamewn_nbc  =  sodirwn  + "/pr.wn.nbc.%04d.%02d.%s.sa.one"   %(year, mon, prtype)
              snamewn_ot   =  sodirwn  + "/pr.wn.ot.%04d.%02d.%s.sa.one"    %(year, mon, prtype)

              # overlap: baroclinic
              sname_te  =  sodir  + "/pr.te.%04d.%02d.%s.sa.one" %(year, mon, prtype)
              sname_tf  =  sodir  + "/pr.tf.%04d.%02d.%s.sa.one" %(year, mon, prtype)
              sname_ef  =  sodir  + "/pr.ef.%04d.%02d.%s.sa.one" %(year, mon, prtype)
              sname_tef =  sodir  + "/pr.tef.%04d.%02d.%s.sa.one" %(year, mon, prtype)
              ...
              # overlap: baroclinic
              a2pr_te.tofile(sname_te)
              a2pr_tf.tofile(sname_tf)
              a2pr_ef.tofile(sname_ef)
              a2pr_tef.tofile(sname_tef)
