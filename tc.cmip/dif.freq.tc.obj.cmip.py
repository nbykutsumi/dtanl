...
for futexpr, model, season in llkey:
  ...
  #---------------
  idir_fut = "/media/disk2/out/CMIP5/sa.one.%s.%s/6hr/tc/freq.%02dh/%04d-%04d.%s"%(model,futexpr,thdura,iyear_fut,eyear_fut,season)
  idir_his = "/media/disk2/out/CMIP5/sa.one.%s.%s/6hr/tc/freq.%02dh/%04d-%04d.%s"%(model,"historical",thdura,iyear_his,eyear_his,season)
  
  iname_fut= idir_fut + "/freq.tc.%s.wc%4.2f.sst%02d.wind%02d.vor%.1e.%s.rad%04dkm.%04d-%04d.%s.sa.one"%(model, thwcore, thsst-273.15, thwind, thrvort, ens, countrad,iyear_fut,eyear_fut,season)
  iname_his= idir_his + "/freq.tc.%s.wc%4.2f.sst%02d.wind%02d.vor%.1e.%s.rad%04dkm.%04d-%04d.%s.sa.one"%(model, thwcore, thsst-273.15, thwind, thrvort, ens, countrad,iyear_his,eyear_his,season)
  ...
  #-- write ------
  sodir      = idir_fut
  soname     = sodir + "/dif.freq.tc.%s.wc%4.2f.sst%02d.wind%02d.vor%.1e.%s.rad%04dkm.%04d-%04d.%s.sa.one"%(model, thwcore, thsst-273.15, thwind, thrvort, ens, countrad,iyear_fut,eyear_fut,season)
  ...
  #----------
  stitle   = "freq. %s.TC season:%s %04d-%04d %s"%(model,season,iyear_fut, eyear_fut,ens)
  #mycm     = "Spectral"
  mycm     = "RdBu_r"
  datname  = soname
