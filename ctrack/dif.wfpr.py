...
for iclass in lclass:
  ...
  for era in lera:
    ...
    #---
    da3rnum[era]   = da3num[era] / da3snum[era]
    da3rnum[era]   = da3rnum[era].filled(0.0)
    #----------------------
    #  num(iclass, wbin) per num of all cyclone class, all wbin
    #---
    da3rtotnum[era]= da3num[era] / ma.masked_equal( da3totnum[era], 0.0)
    da3rtotnum[era]= da3rtotnum[era].filled(0.0)
    #----------------------
    da3mp[era]     = da3sp[era] / ma.masked_equal( da3num[era], 0.0)
...
for iclass in lclass:
  ...
  # X= PDF(Ci)
  #------------------
  da2c_rn[iclass]  = a3num[0] / ma.masked_equal( a2totnum_his, 0.0)
  da2c_rn[iclass]  = da2c_rn[iclass].filled(0.0)

  #------------------
  # Y= PDF(wi|Ci)
  #------------------
  da3pdf_w[iclass] = a3num/ ma.masked_equal( a3snum, 0.0)
  da3pdf_w[iclass] = da3pdf_w[iclass].filled(0.0)

  #------------------
  # Z= P(wi|Ci)
  #------------------
  da3w_p[iclass]   = a3sp / ma.masked_equal( a3num, 0.0)  # devide by "a3num"
...
array(frac_a2ndXYZ, float32).tofile(dscalename["frac.dpdf_c"])
array(frac_a2nXdYZ, float32).tofile(dscalename["frac.dpdf_w"])
array(frac_a2nXYdZ, float32).tofile(dscalename["frac.dp_w"])
##-----------
#*************************************
# make pict
#*************************************
...
stempname =  iname + "temp"
a.tofile(stempname)
#------
cptfile = oekakidir + "/cpt/polar.-3.3.cpt"
pngname = iname[:-3] + ".png"
stitle = "dif"
scalestep = 1.0
overscale = 1
...
os.system("python %s %s %s %s %s %s %s"\
        %(cmd, stempname, cptfile, pngname, stitle, scalestep, overscale))
