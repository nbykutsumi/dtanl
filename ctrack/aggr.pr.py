...
for i in range(len(lclass)):
  daggname[i] = outdir + "/aggr.pr.c%02d_%s_%s_%s_%s_%s.bn"%(season, tstp, model, expr, ens)
#**********************
#  read orography
#----------------------
  ...
  for mon in lmon:
    ...
    #for day in range(1, 1+1):
      for hour in range(0, endh+1, hinc):
        ...
        pgraddir   = lifedir_root     + "/%s/%s/%s/pgrad/%04d"%(model, expr, ens, year)
        prdir      = lifedir_root     + "/%s/%s/%s/pr/%04d"%(model, expr, ens, year)
        ##----------
        ##   names
        ##**********
        ...
        pgradname    = lifedir        + "/pgrad_%s_%s_%s_%s_%s.bn"%(tstp, model, expr, ens, stimeh)
        prname       = lifedir        + "/pr_%s_%s_%s_%s_%s.bn"%(tstp, model, expr, ens, stimeh)
        ##***************************************
        ##   read
        ##---------------------------------------
        a2life   = fromfile(lifename, int32).reshape(ny, nx)
        a2pgrad  = fromfile(pgardname, float32).reshape(ny, nx)
