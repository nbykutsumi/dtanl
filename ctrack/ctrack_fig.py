...
#********************************************************
#********************************************************
def mk_pict_saone_reg_symm_dotshade(a2in, bnd=False, mycm="jet", soname="./temp.png", stitle=False, cbarname=False, miss=-9999.0, lllat=-89.5, lllon=0.5, urlat=89.5, urlon=359.5, a2shade=False, coef=1.0, lonlatfontsize=10.0, lonrotation=90, dotcolor="k", dotstep=1):
  #-- prep ----------------
  plt.clf()
  dlat    = 1.0
  dlon    = 1.0
  ...
  if type(a2shade) != bool:
    ...
    ny_trans      = ydom_saone_last - ydom_saone_first +1
    nx_trans      = xdom_saone_last - xdom_saone_first +1
    a1xplot       = range(nx_trans)[::dotstep]
    a1yplot       = range(ny_trans)[::dotstep]
    print a1xplot
    print a1yplot
    for iix,iiy in [[x,y] for x in a1xplot for y in a1yplot]:
