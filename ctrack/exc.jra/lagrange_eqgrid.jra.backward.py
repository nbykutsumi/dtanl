      ...
      for mon in lmon:
        ...
        #for day in range(28, eday+1):
          if ((vtype == "GPCP1DD")&((year,mon,day) == (1997,1,1))):
            continue
          print "exc",vtype, year, mon, day, "eday=",eday
          print "lmon=" , lmon
          for hour in lhour:
        ...
        #--- name --------------
        if bsttc == True:
          odir         = "/media/disk2/out/JRA25/sa.one.anl_p/composite/exc.bsttc.%s.%s.%s/%04d%02d"%(var,vtype,region, year, mon)
        elif bsttc == False:
          odir         = "/media/disk2/out/JRA25/sa.one.anl_p/composite/exc.objtc.%s.%s.%s/%04d%02d"%(var,vtype,region, year, mon)
        #
        ...
        print a2sum_eqgrid.max(), oname_sum
        ##-- figure ---------------------
        #
        #if var == "pr":
        #  #if vtype in ["GPCP1DD"]:
