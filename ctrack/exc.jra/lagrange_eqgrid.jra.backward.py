...
import ctrack_func, ctrack_para, tc_para, tc_func
...
for season in lseason:
  ...
  for vtype in lvtype:
    ...
    if bsttc == True:
      odir         = "/media/disk2/out/JRA25/sa.one.anl_p/composite/exc.bsttc.%s.%04d-%04d"%(region, iyear, eyear)
    elif bsttc == False:
      odir         = "/media/disk2/out/JRA25/sa.one.anl_p/composite/exc.objtc.%s.%04d-%04d"%(region, iyear, eyear)
    #
    ...
    for iclass in lclass:
      ...
      if var == "pr":
        if bsttc == True:
          oname_mean   = odir + "/exc.bsttc.%s.%s.%s.%04.0f-%04.0f.bn"%(var, vtype, region, thgrad_min, thgrad_max)
          oname_cor    = odir + "/cc.exc.bsttc.%s.%s.%s.%04.0f-%04.0f.bn"%(var, vtype, region, thgrad_min, thgrad_max)
        elif bsttc == False:
          oname_mean   = odir + "/exc.objtc.%s.%s.%s.%04.0f-%04.0f.bn"%(var, vtype, region, thgrad_min, thgrad_max)
          oname_cor    = odir + "/cc.exc.objtc.%s.%s.%s.%04.0f-%04.0f.bn"%(var, vtype, region, thgrad_min, thgrad_max)
        else:
          print "check bsttc"
          sys.exit() 
      #---------------------
            ...
            #for day in range(28, eday+1):
              ...
              for hour in lhour:
                ...
                if bsttc == True:
                  ...
                  a2tc    = ones([ny_org,nx_org],float32) * miss_out
                  for xy in lxy:
                    ixpy, iypy = xy
                    ixfort, iyfort  = ixpy+1, iypy+1
                    a2tc[iypy,ixpy] = ctrack_fsub.point_pgrad_rad_saone(ixfort,iyfort,a2psl.T, 300.0, miss_out)
  
                #*****************************
                # TC objective
                #----------------
                diname["tc"]    = didir["tc"]    + "/tc.%04d%02d%02d%02d.sa.one"%(year, mon, day, hour)
                ...
                a2tc            = fromfile(diname["tc"],float32).reshape(ny_org, nx_org)
                ...
                #--- cyclone center data -------------
                da2in["center"] = ma.masked_less(da2in["pgrad"], 0.0).filled(miss_out)
                #--- check duration -----
                ...
                #--- mask out TC --------
                da2in["center"] = ma.masked_where( a2tc !=miss_out, da2in["center"]).filled(miss_out)
