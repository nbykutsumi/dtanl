        ...
        for hour in lhour:
          ...
          for ftype in lftype:
            ...
            for dist_km in ldist_km:
              ...
              if ((ftype==1)&(dist_km == 0.0)):
                prsum_tmp    = ma.masked_equal(a2pr_hs,miss).filled(0.0).sum()*60*60.
                numsum_tmp   = ma.masked_where(a2pr_hs==miss, a2one).filled(0.0).sum()
                print year,mon,day,hour,"prsum=", prsum_tmp, "numsum=",numsum_tmp 
                prtot   = prtot  + prsum_tmp
                numtot  = numtot + numsum_tmp
                sout =  sout + "%s,%s,%s,%s,%s,%s\n"%(year,mon,day,hour,prsum_tmp,numsum_tmp)
              # SUM ---
              ...
              #------------------
      #** output ****************************************
      odir   = "/media/disk2/out/chart/ASAS/front/agg/test/prof"
...
#-------------------------
if window == "no":
  csvname  = odir  + "/pr.auto.csv"
elif window in ["in"]:
  csvname  = odir  + "/pr.auto.in.%04dkm.csv"%(dist_mask)
#
