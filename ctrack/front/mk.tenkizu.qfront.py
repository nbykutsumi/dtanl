...
def mk_front_loc_contour(a2thermo, a2gradthermo, thfmask1, thfmask2):
  ...
  a2loc    = ma.masked_where(a2loc.mask, a2fmask2)
  return a2loc
...
bnd        = [0.0, 0.0001, 0.0003, 0.0005, 0.0007, 0.0009, 0.0011, 0.0013]
scm        = "jet"
stitle     = "%s (K 100km-1)\n"%(vtype)
...
if contflag == "True":
  ...
  #a2loc    = ctrack_fsub.find_highsidevalue_saone(a2gradthermo.T, a2loc.T, a2gradthermo.T, highsidedist, miss_out).T
  ...
  a2loc    = ma.masked_where(a2gradorogmask > thgradorog, a2loc).filled(miss_out)
  a2loc    = dtanl_fsub.del_front_2grids(a2loc.T, miss_out).T
  a2loc    = ctrack_fsub.find_highsidevalue_saone(a2gradtheta_e.T, a2loc.T, a2gradthermo.T, highsidedist, miss_out).T
  mk_regmap(a2loc, bnd, scm, stitle, soname, cbarname, miss_out)
#------
