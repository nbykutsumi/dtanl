        ...
        for mon in lmon:
          ...
          for day in range(iday, eday+1):
            for hour in lhour:
              ...
              tname     = tdir          + "/anl_p.TMP.0850hPa.%04d%02d%02d%02d.sa.one"%(year,mon,day,hour)
              qname     = qdir          + "/anl_p.SPFH.0850hPa.%04d%02d%02d%02d.sa.one"%(year,mon,day,hour)
              ...
              if vtype == "tv":
                a2thermo     = a2t * (1.0+0.61*a2q)
              elif vtype == "theta":
                a2thermo  = dtanl_fsub.mk_a2theta(plev, a2t.T).T
              elif vtype == "theta_e":
                a2thermo_e= dtanl_fsub.mk_a2theta_e(plev, a2t.T, a2q.T).T
              ...
              if vecttype == "grad":
                a2vecx, a2vecy     = ctrack_fsub.mk_a2highsidevector_saone(a2thermo.T, a2chart_seg.T, dist_km*1000.0, miss)
              elif vecttype == "grad2":
                a2vecx, a2vecy     = ctrack_fsub.mk_a2highsidevector_saone(-a2gradthermo.T, a2chart_seg.T, dist_km*1000.0, miss)
              #-----------------------------  
              a2vecx             = a2vecx.T
              a2vecy             = a2vecy.T
              ...
              #-- title -------------------
              stitle = "verctor:%s %s\n"%(vecttype,vtype)
              stitle = stitle + "%04d-%02d-%02d-%02d"%(year,mon,day,hour)
              axmap.set_title(stitle)
    
              #-- save --------------------
              figname = "/media/disk2/temp/highside/chart.highside.%s.%s.%04d.%02d.%02d.%02d.png"%(vecttype,vtype,year,mon,day,hour)
              plt.savefig(figname)
              print figname
