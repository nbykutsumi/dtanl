...
for vtype in lvtype:
  ...
  sodir         = sodir_root + "/%04d%02d/prec.glob"%(year, mon)
  #sodir         = "/home/utsumi/temp"
  ctrack_func.mk_dir(sodir)
  
  soname        = sodir + "/prec.glob.%04d.%02d.%02d.%02d.%s.png"%(year, mon, day, hour, vtype)
  #----------------------------
  dlat    = 1.0
  dlon    = 1.0
  ...
  im       = M.contour(LONS, LATS, a2psl_trans, latlon=True, linewidth=0.2,levels=llevels,  colors="gray")
  ...
  for iclass in lclass[1:]:
    ...
    for latlon in dcenter[iclass]:
      lat = latlon[0]
      lon = latlon[1]
      M.scatter( lon, lat, color="r", marker="o", s=50)
