...
for season in lseason:
      ...
      for year in range(iyear, eyear+1):
        for mon in lmon:
          ...
          if window == "no":
            ...
            iname_pr   =  idir + "/pr.maskrad.%s.%04dkm.%04dkm.%s.sa.one"%(window, dist_mask, dist_km, ftype)
            iname_num  =  idir + "/num.maskrad.%s.%04dkm.%04dkm.%s.sa.one"%(window, dist_mask, dist_km, ftype)
          #--
          ...
          for plev in lplev:
            ...
            #-------------------------
            if window == "no":
              iname_theta         =  idir + "/theta.maskrad.%04dkm.%s.%04dhPa.sa.one"%(dist_km, ftype, plev)
              iname_num_theta     =  idir + "/num_theta.maskrad.%04dkm.%s.%04dhPa.sa.one"%(dist_km, ftype, plev)
            elif window in ["in","out"]:
              iname_theta         =  idir + "/theta.maskrad.%s.%04dkm.%04dkm.%s.%04dhPa.sa.one"%(window, dist_mask, dist_km, ftype, plev)
              iname_num_theta     =  idir + "/num_theta.maskrad.%s.%04dkm.%04dkm.%s.%04dhPa.sa.one"%(window, dist_mask, dist_km, ftype, plev)
            #--
          ...
          if window == "no":
            ...
            iname_gradtheta         =  idir + "/grad.theta.maskrad.%s.%04dkm.%04dkm.%s.sa.one"%(window, dist_mask, dist_km, ftype)
            iname_num_gradtheta     =  idir + "/num_grad.theta.maskrad.%s.%04dkm.%04dkm.%s.sa.one"%(window, dist_mask, dist_km, ftype)
          #--
          ...
          if window == "no":
            ...
            iname_grad2theta         =  idir + "/grad2.theta.maskrad.%s.%04dkm.%04dkm.%s.sa.one"%(window,dist_mask, dist_km, ftype)
            iname_num_grad2theta     =  idir + "/num_grad2.theta.maskrad.%s.%04dkm.%04dkm.%s.sa.one"%(window,dist_mask, dist_km, ftype)
          #--
  ...
  for ftype in lftype:
    ...
    if window == "no":
      ...
      figname   = pictdir + "/cont.dist.theta.maskrad.%s.%04dkm.%04d-%04d.%s.%s.%s.png"%(window, dist_mask, iyear,eyear,season,region,ftype)
  ...
  if window == "no":
    ...
    figname   = pictdir + "/plt.dist.grad.theta.maskrad.%s.%04dkm.%04d-%04d.%s.%s.png"%(window, dist_mask, iyear,eyear,season,region)
  figplot.savefig(figname)
  print figname
  ...
  if window == "no":
    ...
    figname   = pictdir + "/plt.dist.grad2theta.maskrad.%s.%04dkm.%04d-%04d.%s.%s.png"%(window, dist_mask, iyear,eyear,season,region)
  figplot.savefig(figname)
  print figname
