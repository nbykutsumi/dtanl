...
#***************************************
def locbase2locbaiu(a2loc_base, a2fmask1, a2fmask2, a2q, a2u850, a2uup, thbmask1, thbmask2, qmin, u850min, uup_min):
  a2loc    = a2loc_base
  a2loc    = ma.masked_where(a2fmask1 < thbmask1, a2loc)
  a2loc    = ma.masked_where(a2fmask2 < thbmask2, a2loc)
...
for lthfmask in llthfmask:
  ...
  for year in range(iyear, eyear+1):
    ...
    for mon in lmon:
      ...
      a2count_nobc_front_mon   = zeros([ny,nx],float32)
      a2count_onbaiu_mon       = zeros([ny,nx],float32)  
      a2count_bcf_onbaiu_mon   = zeros([ny,nx],float32)  
      a2count_nobc_onbaiu_mon  = zeros([ny,nx],float32)  
      #************************
      #-- init for baiu ------
      a2pr_baiu_mon            = zeros([ny,nx],float32)
      a2count_baiu_mon         = zeros([ny,nx],float32)  
      #************************
      #-- init for olap ------
      ...
      for day in range(iday, eday+1):
        ...
        for hour in lhour:
          ...
          #-----------------------------
          # objective baiu territory
          #-----------------------------
          a2loc_baiu   = locbase2locbaiu(a2loc_base, a2fmask1, a2fmask2, a2q, a2u850, a2uup, thbmask1, thbmask2, qmin, u850min, uup_min)
          a2loc_baiu   = ctrack_fsub.find_highsidevalue_saone(a2gradthermo.T, a2loc_baiu.T, a2gradtv.T, distwarmside, miss).T
  
          #-- count baiu loc  --
          a2count_baiu_mon = a2count_baiu_mon + ma.masked_where(a2loc_baiu == miss,a2one).filled(0.0)
  
          #-- count only-baiu loc ----------------
          a2temp         = ma.masked_where(a2loc_baiu == miss, a2one)
          a2temp         = ma.masked_where(a2loc_front != miss, a2temp)
          a2count_onbaiu_mon = a2count_onbaiu_mon + a2temp.filled(0.0)
  
          #-- count non-baloclinic only-baiu loc --
          a2temp         = ma.masked_where(a2loc_baiu ==miss, a2one)
          a2temp         = ma.masked_where(a2loc_baiu >= thbc,   a2temp)
          a2temp         = ma.masked_where(a2loc_front != miss, a2temp)
          a2count_nobc_onbaiu_mon = a2count_nobc_onbaiu_mon + a2temp.filled(0.0)
          
          #----------------------------------------
          a2terr_baiu = ctrack_fsub.mk_territory_saone( a2loc_baiu.T, thdist*1000.0, miss, -89.5, 1.0, 1.0).T
          ...
          #***********************************************
          # aggregate precip : baiu
          #-----------------------------------
          a2pr_baiu_temp  = ma.masked_where(a2terr_baiu ==miss, a2pr)
          #---------------------
          a2pr_baiu_temp  = a2pr_baiu_temp.filled(0.0)
          #--- add -------------
          a2pr_baiu_mon   = a2pr_baiu_mon + a2pr_baiu_temp
          #************************************************
