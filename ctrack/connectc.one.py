...
print expr       #  = sys.argv[2]
print ens        #  = sys.argv[3]
print tstp       #  = sys.argv[4]
print hinc       #  = int(sys.argv[5])
print iyear      #  = int(sys.argv[6])
print eyear      #  = int(sys.argv[7])
print imon       #  = int(sys.argv[8])
print emon       #  = int(sys.argv[9])
print nx         #  = int(sys.argv[10])
print ny         #  = int(sys.argv[11])
print miss_dbl   #  = float(sys.argv[12])
print miss_int   #  = int(sys.argv[13])
print endh       #  = int(sys.argv[14])
print thdp       #  = float(sys.argv[15])
print thdist     #  = float(sys.argv[16])
...
winddir_root    = "/media/disk2/out/CMIP5/sa.one/day"
axisdir_root    = psldir_root
#
lastposdir_root  = "/media/disk2/out/CMIP5/sa.one/%s"%(tstp)
#pmindir_root    = "/media/disk2/out/CMIP5/sa.one/%s"%(tstp)
pgmaxdir_root   = "/media/disk2/out/CMIP5/sa.one/%s"%(tstp)
iposdir_root    = "/media/disk2/out/CMIP5/sa.one/%s"%(tstp)
idatedir_root   = "/media/disk2/out/CMIP5/sa.one/%s"%(tstp)
timedir_root    = "/media/disk2/out/CMIP5/sa.one/%s"%(tstp)
pgraddir_root   = "/media/disk2/out/CMIP5/sa.one/%s"%(tstp)
lifedir_root    = "/media/disk2/out/CMIP5/sa.one/%s"%(tstp)
nextposdir_root = "/media/disk2/out/CMIP5/sa.one/%s"%(tstp)
#****************************************************
# read lat, lon data
#----------------------
  ...
  #for mon in range(1, 1+1):
    ...
    #for day in range(1, 1+1):
      for hour in range(0, endh+1, hinc):
        ...
        #   names
        #**********
        pslname0    = psldir0   + "/psl_%sPlev_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh0)
        pmeanname0  = pmeandir0 + "/pmean_%s_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh0)
        uaname0     = uadir0    + "/run.mean.ua_day_%s_%s_%s_%s.700hPa.sa.one"%(model, expr, ens, stimed0)
        vaname0     = vadir0    + "/run.mean.va_day_%s_%s_%s_%s.700hPa.sa.one"%(model, expr, ens, stimed0)
        #
        lastposname0 = lastposdir0     + "/lastpos_%s_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh0) 
        #pminname0    = pmindir0        + "/pmin_%s_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh0) 
        pgmaxname0   = pgmaxdir0       + "/pgmax_%s_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh0) 
        iposname0    = iposdir0        + "/ipos_%s_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh0)
        idatename0   = idatedir0       + "/idate_%s_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh0)
        timename0    = timedir0           + "/time_%s_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh0)
        #
        mk_dir(lastposdir0)
        #mk_dir(pmindir0)
        mk_dir(pgmaxdir0)
        ...
        #   names
        #**********
        pslname1   = psldir1           + "/psl_%sPlev_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh1)
        pmeanname1 = pmeandir1         + "/pmean_%s_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh1)
        #
        lastposname1 = lastposdir1     + "/lastpos_%s_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh1) 
        #pminname1    = pmindir1        + "/pmin_%s_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh1) 
        pgmaxname1    = pgmaxdir1       + "/pgmax_%s_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh1) 
        iposname1    = iposdir1        + "/ipos_%s_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh1)
        idatename1   = idatedir1       + "/idate_%s_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh1)
        timename1    = timedir1        + "/time_%s_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh1)
        pgradname1   = pgraddir1       + "/pgrad_%s_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh1)
        #***************************************
        # read data
        #---------------------------------------
        #   for 0
        ...
        a2lastpos1 = array(ctrackout[0].T, int32)  # lastpos is in fortran system, not python system (returns nx*(iy-1) + ix,  nx = 1,2,.., ny=1,2,..)
  ...
  for mon in range(emon, imon -1, -1):
    ...
    for day in range(ed, 1-1, -1):
      for hour in range(endh, 0-1, -hinc):
        ...
        # out name
        #**********
        lifename1     = lifedir1          + "/life_%s_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh1)
        nextposname0  = nextposdir0       + "/nextpos_%s_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh0)
        ...
        if counter == 1:
          ...
          nextposname1  = nextposdir1     + "/nextpos_%s_%s_%s_%s_%s.sa.one"%(tstp, model, expr, ens, stimeh1)
