...
lthfmask1 = [0.03, 0.15, 0.3]
#lthfmask1 = [0.00, 0.03, 0.05]
#thfmask1 = 0.0
#thfmask1 = 0.1
#thfmask1 = 0.2
#thfmask1 = 0.3
...
for plev in lplev:
  ...
  a2gradthermo = dtanl_fsub.mk_a2grad_abs_saone(a2q.T).T
  a2thermo = a2q
  #-- locator ---
  ...
  soname        = sodir + "/grad.q.tenkizu.%04d.%02d.%02d.%02d.%04dhPa.%s.png"%(year, mon, day, hour, plev*0.01, vtype)
  ...
  #--- value ------------------
  a2v           = a2gradthermo * 1000.0*100.0 *1.0e+3      # [g kg-1 (100km)-1]
  a2loc         = a2loc * (1000.0*100.0)**3.0 *1.0e+3    # [g kg-1 (100km)-3]
  a2fmask1      = a2fmask1 * (1000.0*100.0)**2.0 *1.0e+3 # [g kg-1 (100km)-2]
  a2fmask2      = a2fmask2 * (1000.0*100.0) *1.0e+3      # [g kg-1 (100km)-1]
  #
  da2v[plev]   = a2v
  da2loc[plev] = a2loc
  #----------------------------
  ...
  #-- boundaries ----------
  bnd        = [0.1, 0.4, 0.7, 1.0, 1.3, 1.6, 1.9]
  ...
  for thfmask0 in lthfmask0:
    for thfmask2 in lthfmask2:
      #---------------------------
      name_loc_full = sodir[:-3] + "/loc.q.tenkizu.%04d.%02d.%02d.%02d.%04dhPa.%s.M0_%3.2f.M2_%3.2f.join.png"%(year, mon, day, hour, plev*0.01, vtype, thfmask0, thfmask2)
      #
      nsub_x    = 3
      nsub_y    = 3
      subsize_x = 800
      ...
      for thfmask1 in lthfmask1:
        ...
        name_loc      = sodir + "/loc.q.tenkizu.%04d.%02d.%02d.%02d.%04dhPa.%s.M0_%3.2f.M1_%3.2f.M2_%3.2f.temp.png"%(year, mon, day, hour, plev*0.01, vtype, thfmask0, thfmask1, thfmask2)
        #
        #-- boundaries ----------
        ...
        M.drawparallels(arange(-90.0,90.0, parallels), labels=[1, 0, 0, 0])
        #-- title -------------------
        axmap.set_title("%04d-%02d-%02d  UTC %02d:00 (JST %02d:00) %04dhPa \n M0 %3.2f, M1 %3.2f, M2 %3.2f"%(year, mon, day, hour, hour+9, plev*0.01, thfmask0, thfmask1, thfmask2))
