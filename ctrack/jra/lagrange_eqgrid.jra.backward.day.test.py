...
#--- land sea mask ---
if aphromask == "TRUE":
  maskdir  = "/media/disk2/data/aphro/MA/sa.one/2004"
  maskname = maskdir + "/APHRO_MA_025deg_V1101.2004.01.01.sa.one"
  a2aphromask   = fromfile(maskname, float32).reshape(70,90)
  a2aphromask   = aphro_func.aphro2global_one(a2aphromask, "MA", miss_out)
  a2aphromask   = ma.masked_not_equal(a2aphromask, miss_out).filled(1.0)
#---------------------
...
for vtype in lvtype:
  ...
  for iclass in lclass:
    ...
    #---- dummy for count --------------------
    a2count_eqgrid    = zeros([ny_eqgrid, nx_eqgrid], float32)

    #---- dummy for max ----------------------
    a2max_eqgrid      = zeros([ny_eqgrid, nx_eqgrid], float32)

    #---- dummy for mean global --------------
    a2one_glob        = ones([180,360], float32)
    a2sum_glob        = zeros([180,360], float32)
    a2num_glob        = zeros([180,360], float32)
    #-----------------------------------------
    ...
    #---------------------
    if var == "pr":
      oname_mean   = odir + "/%s.day.%s.%s.%04.0f-%04.0f.bn"%(var, vtype, sreg, thgrad_min, thgrad_max)
      oname_count  = odir + "/count.day.%s.%s.%04.0f-%04.0f.bn"%(vtype, sreg, thgrad_min, thgrad_max)
      oname_cor    = odir + "/cc.day.%s.%s.%s.%04.0f-%04.0f.bn"%(var, vtype, sreg, thgrad_min, thgrad_max)
      oname_max    = odir + "/max.day.%s.%s.%s.%04.0f-%04.0f.bn"%(var, vtype, sreg, thgrad_min, thgrad_max)
      oname_globmean = odir + "/mean.glob.day.%s.%s.%s.bn"%(var, vtype, sreg)
            ...
            for iy_org in ly_org:
              for ix_org in lx_org:
                ...
                if thgrad_min <= da2in["pgrad"][iy_org, ix_org] < thgrad_max:
                  ...
                  #--- for count --------
                  a2count_eqgrid_temp  = a2num_eqgrid_temp
    ...
    #-- global ------------
    a2mean_glob    = ma.masked_where(a2num_glob==0.0, a2sum_glob)/a2num_glob
    a2mean_glob    = a2mean_glob.filled(miss_out)
 
    #-- save --------------
    a2mean_eqgrid  = a2mean_eqgrid * coef
    a2max_eqgrid   = a2max_eqgrid *coef
    #
    a2mean_eqgrid.tofile(oname_mean)
    a2count_eqgrid.tofile(oname_count)
    a2max_eqgrid.tofile(oname_max)
    a2mean_glob.tofile(oname_globmean)
