    ...
    do iy=1,ny
      do ix=1,nx
        if ((r2pr(ix,iy) .gt. r2pr_lw(ix,iy)) .and. (r2pr(ix,iy) .le. r2pr_up(ix,iy)) ) then 
          if ( r3in(ix,iy,iz) .ne. rmiss) then
            r3count(ix,iy,iz) = r3count(ix,iy,iz) + 1.0
            if (r3stck(ix,iy,iz) .eq. rmiss) then
              r3stck(ix,iy,iz)  = r3in(ix,iy,iz)
            else
              r3stck(ix,iy,iz)  = r3stck(ix,iy,iz)  + r3in(ix,iy,iz)
            end if
          end if
        end if
      end do
...
do iz=1,nz
  do iy=1,ny
    do ix=1,nx
      if ( r3count(ix,iy,iz) .eq. 0.0) then
        r3mean(ix,iy,iz) = rmiss
      else
        r3mean(ix,iy,iz) = r3stck(ix,iy,iz) / r3count(ix,iy,iz)
        if ((ix.eq.30).and.(iy.eq.30).and.(iz.eq.1))then
          print *,r3mean(ix,iy,iz),r3stck(ix,iy,iz),r3count(ix,iy,iz)
        endif
      end if
    end do
  end do
