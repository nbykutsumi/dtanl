...
for year in range(iyear, eyear+1):
  ...
  for mon in range(im, em+1):
    ...
    for day in range(1, calendar.monthrange(year,mon)[1] +1):
      ...
      # set prec files
      #---------------------------
      sprec_reanl = idir_root + "/%s/%04d/%s.%s.%s.%04d%02d%02d00.bn"\
             %(dvname["pr"], year, model, tstp, dvname["pr"], year, mon, day)
      #--
      aprec_reanl = fromfile(sprec_reanl, float32).reshape(ny,nx)
      #---------------------------
      #- files for variables 
      #---------------------------
      for var in lvar:
      ...
      for iy in range(0,ny):
        ...
        for ix in range(0,nx):
          ...
          #----------------------------
          prec_reanl = aprec_reanl[iy,ix]
          #if (prec_reanl < aprec_lw[iy,ix]) or (aprec_up[iy,ix] <= prec_reanl)  :
          #  continue 
          #----------------------------
          lat  = llat[iy]
          lon  = llon[ix]
...
mk_dir(odir)
#
sraw_list = odir + "/%s.rawlist.%04d-%04d.%02d-%02d.csv"%(model, iyear, eyear, im, em)
