...
for region in lregion:
  idir      = idir_root + "/tc.dt/%s"%(region)
  dlfrac    = {}
  dlv       = {}
  for model in lmodel:
    iname  = idir + "/dt.%s.%04d-%04d.%s.csv"%(model,iyear,eyear,region)
  ...
  for i in range(len(lfrac)):
    ...
    sout = sout + sout_temp +"\n"
  #--------------------------
  soname = idir + "/dt.%04d-%04d.%s.csv"%(iyear,eyear,region)
  ...
  #--- vort ----------
  #-----------------------
  idir      = idir_root + "/tc.vort/%s"%(region)
  dlfrac    = {}
  dlv       = {}
  for model in lmodel:
    iname  = idir + "/vort.%s.%04d-%04d.%s.csv"%(model,iyear,eyear,region)
    f = open(iname, "r"); lines=f.readlines(); f.close()
    #
    dlfrac[model]  = []
    dlv[model]     = []
  ...
  for i in range(len(lfrac)):
    ...
    sout = sout + sout_temp +"\n"
  #--------------------------
  soname = idir + "/vort.%04d-%04d.%s.csv"%(iyear,eyear,region)
  f=open(soname,"w"); f.write(sout); f.close()
  print soname
