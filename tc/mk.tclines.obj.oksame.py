...
if len(sys.argv) > 1:
  ...
  try:
    ...
    season  = season
  #-----
  thdura    = int(sys.argv[4])
  thwcore   = float(sys.argv[5])
  thsst     = float(sys.argv[6])
  thwind    = float(sys.argv[7])
  thrvort   = float(sys.argv[8])
  plev_low  = float(sys.argv[9])
...
lifedir_root    = "/media/disk2/out/JRA25/sa.one.%s.oksame/6hr/life"%(model)
nextposdir_root = "/media/disk2/out/JRA25/sa.one.%s.oksame/6hr/nextpos"%(model)
lastposdir_root = "/media/disk2/out/JRA25/sa.one.%s.oksame/6hr/lastpos"%(model)
iposdir_root    = "/media/disk2/out/JRA25/sa.one.%s.oksame/6hr/ipos"%(model)
#
...
soname  = sodir + "/oksame.tclines.%s.%04d-%04d.%s.%02dh.wc%3.2f.sst%d.wind%d.vor%.1e.tplow%d.pup%d.png"%(model, iyear, eyear,season, thdura, thwcore, thsst -273.15, thwind, thrvort, tplev_low*0.01, plev_up*0.01)
plt.savefig(soname)
plt.clf()
print soname
plt.clf()
