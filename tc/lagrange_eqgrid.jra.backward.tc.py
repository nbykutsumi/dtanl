...
import tc_func
import ctrack_func, ctrack_para, tc_para
...
for season in lseason:
  ...
  for vtype in lvtype:
    ...
    if bsttc == True:
      odir         = "/media/disk2/out/JRA25/sa.one.anl_p/composite/tc.bst.%s.%04d-%04d"%(region,iyear,eyear)
    elif bsttc == False:
      odir         = "/media/disk2/out/JRA25/sa.one.anl_p/composite/tc.obj.%s.%04d-%04d"%(region,iyear,eyear)
    else:
    ...
    for iclass in lclass:
      ...
      if var == "pr":
        if bsttc == True:
          oname_mean   = odir + "/tc.bst.%s.%s.%s.%s.%04.0f-%04.0f.bn"%(var, vtype, region, season, thgrad_min, thgrad_max)
          oname_cor    = odir + "/cc.tc.bst.%s.%s.%s.%s.%04.0f-%04.0f.bn"%(var, vtype, region, season, thgrad_min, thgrad_max)
        if bsttc == False:
          oname_mean   = odir + "/tc.obj.%s.%s.%s.%s.%04.0f-%04.0f.bn"%(var, vtype, region, season, thgrad_min, thgrad_max)
          oname_cor    = odir + "/cc.tc.obj.%s.%s.%s.%s.%04.0f-%04.0f.bn"%(var, vtype, region, season, thgrad_min, thgrad_max)
