  ...
  for expr in lexpr:
    ...
    for var in lvar:
      ...
      for y in range(iy, ey+1):
        print y
        dir_pr = idir_root + "/%s/%04d"%(mk_dir_tail("pr", tstp, model,expr,ens), y)
        dir_prc  = idir_root + "/%s/%04d"%(mk_dir_tail("prc", tstp, model,expr,ens), y)
        dir_Tsfc = idir_root + "/%s/%04d"%(mk_dir_tail("tas", tstp, model,expr,ens), y)
        dir_qsfc = idir_root + "/%s/%04d"%(mk_dir_tail("huss", tstp, model,expr,ens), y)
        dir_zsfc = idir_root + "/%s"%(mk_dir_tail("orog", "fx", model,expr,"r0i0p0"))
        dir_Psea = idir_root + "/%s/%04d"%(mk_dir_tail("psl", tstp, model,expr,ens), y)
        dir_zg = idir_root + "/%s/%04d"%(mk_dir_tail("zg", tstp, model,expr,ens), y) 
        dir_wap = idir_root + "/%s/%04d"%(mk_dir_tail("wap", tstp, model,expr,ens), y) 
        dir_hur = idir_root + "/%s/%04d"%(mk_dir_tail("hur", tstp, model,expr,ens), y) 
        dir_ta  = idir_root + "/%s/%04d"%(mk_dir_tail("ta" , tstp, model,expr,ens), y) 
        dir_rhs = idir_root + "/%s/%04d"%(mk_dir_tail("rhs", tstp, model,expr,ens), y) 
        #--------------------
