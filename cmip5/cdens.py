  ...
  for mon in lmon:
    ...
    #for day in range(1, 1+1):
      for hour in range(0, endh+1, hinc):
        ...
        a2dura     = tsolvelife[1].T
        #---------------------------------------
        # filter
        #*******************
        a2center   = array(ones(ny*nx).reshape(ny, nx), int32)
        a2center   = ma.masked_where(a2dura < thdura, a2center)
        a2center   = ma.masked_where(a2pmin > thpmin, a2center)
        a2center   = a2center.filled(0)
        #-------------------
        a2dens     = a2dens + a2center
#----------
a2dens_area = a2dens / a2area /(eyear - iyear +1)
a2track     = ma.masked_greater(a2dens, 0.0).filled(1.0)
#***********************
# names
#-----------------------
mapdir    = "/media/disk2/out/CMIP5/6hr/%s/%s/%s/tracks/map"%(model, expr, ens)
dens_area_name = mapdir + "/dens.area_%s_%s_%s_%s_%s.bn"%(season,model, expr, ens, stimeh)
track_name     = mapdir + "/track.grid_%s_%s_%s_%s_%s.bn"%(season,model, expr, ens, stimeh)
#-----------------------
# write 
#***********************
