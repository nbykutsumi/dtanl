...
!###########################################################
character*128         cPsfc, cTsfc, cRH
!
real                  rPsfc, rTsfc, rRH ! rPsfc: [Pa]
real                  res, rqsfc
real                  lcl
...
!***************************************
if (iargc().lt.3) then
  print *, "Usage: cmd [Psfc] [Tsfc] [RH]"
  print *, "Psfc: (Pa),  Tsfc: (K), RH: (%)"
  stop
endif
!
call getarg(1, cPsfc)
...
res = cal_es(rTsfc)   ! Pa
res = res * 0.01      ! Pa -> hPa
rPsfc = rPsfc * 0.01  ! Pa -> hPa
rqsfc = epsi * res * rRH / (rPsfc - res * rRH)

dPsfc_hPa = dble(rPsfc)
dTsfc = dble(rTsfc)
dq    = dble(rqsfc)
!-------------
x=1000.d0
